...
proc cliopts_set_list_files_or_dirs(list_path_objects: var seq[string], i: var int, total_param: int) =
  if i + 1 > total_param:
  ...
  while i <= total_params_count:
    ...
    if currentParam.startsWith("-"):
      ...
      of "--yrdb": # Short alias of --path-yaradb
        cliopts_set_db_path_yara(options, i, total_params_count):
      of "--scan-files":
        cliopts_set_list_files_or_dirs(options.list_path_objects, i, total_params_count)
      of "--scan-procs":
